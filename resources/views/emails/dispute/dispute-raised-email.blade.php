...
<body>
    <div class="reset-wrapper">
        ...
        <h1 class="title"><svg version="1.1" id="Capa_1" x="0px" y="0px" width="45.311px" height="45.311px" viewBox="0 0 45.311 45.311" style="enable-background:new 0 0 45.311 45.311;" xml:space="preserve">
            ...
            </svg> Transaction Dispute</h1>
        <p>You are receiving this mail because you are a stakeholder on a disputed transaction on the CHF Program.</p>
        <h3><strong>Transaction Detail: </strong></h3>
        <p><strong>Transaction ID: </strong> {{$transaction_dispute->transaction_id}}</p>
        <p><strong>Hospital: </strong> {{$transaction_dispute->transactions[0]->coe->coe_name}} </p>
        <p><strong>COE Staff: </strong> {{$transaction_dispute->transactions[0]->biller->last_name}}
            {{$transaction_dispute->transactions[0]->biller->first_name}}
        </p>
        <p><strong>Patient ID: </strong> {{$transaction_dispute->transactions[0]->user->patient->chf_id}} </p>
        <p><strong>Transaction Date: </strong> {{$transaction_dispute->transactions[0]->created_at}} </p>
        <p><strong>Dispute: </strong> {{$transaction_dispute->reason_for_dispute}} </p>
        <h3>Services:</h3>
        @php
        $total = 0;
        @endphp
        @foreach($transaction_dispute->transactions as $transaction)
        @php
        $total += $transaction->total;
        @endphp
        @if($transaction->is_drug)
        <p class="service">
            @foreach($drugs as $drug)
            @if($drug['productId'] == $transaction->drug_id)
            {{$drug['productName']}} ({{$drug['description']}}) - {{$transaction->quantity}} -
        ...
        @else
        <p class="service">{{$transaction->service->service_name}} - {{$transaction->quantity}} -
            NGN{{$transaction->total}}
        </p>
        ...
        @endforeach
        <hr>
        <p class="text-right"><strong>Total</strong> NGN{{$total}}</p>
        <a href="https://chf.emgeresources.com/dashboard" class="btn"> Login to CHF here!</a>
        <p class="foot-note"></p>
        <p class="foot-note">
            <span class="d-block">Federal Secretariat Complex, Phase III, Shehu Shagari Way, Central Business District.
                Abuja.</span>
            &copy; CHF 2021
        </p>
    </div>
